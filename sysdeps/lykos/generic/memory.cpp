...
namespace mlibc
{
    ...
    {
        sys_vm_map(NULL, size, PROT_WRITE | PROT_READ | PROT_EXEC, MAP_ANON, -1, 0, pointer);
        return 0;
    }
    ...
    {
        *window = (void*)syscall6(SYSCALL_MMAP, (uint64_t)hint, (uint64_t)size, (uint64_t)prot, (uint64_t)flags, (uint64_t)fd, (uint64_t)offset);
        return 0;
    }
